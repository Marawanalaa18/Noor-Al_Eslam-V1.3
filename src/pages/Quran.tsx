...
import { BookOpen, Search, ChevronLeft, ChevronRight, X, Maximize, Minimize, Bookmark, Play, Pause, FastForward, Settings, Volume2, AlertCircle, Moon } from 'lucide-react';
...
export function Quran() {
  const { data: quranData, loading, error, refetch } = useQuranData();
  ...
  const [toolbarPosition, setToolbarPosition] = useState<{ top: number, bottom: number, left: number } | null>(null);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      const results = quranData.ayahs.filter(ayah => {
        const normalizedAyah = ayah.normalizedText || normalizeArabic(ayah.text);
              ...
              <div className="flex items-center gap-4 w-full md:w-auto">
                <div className="flex items-center gap-2 bg-primary/5 dark:bg-gold/5 rounded-full p-1 border border-primary/10 dark:border-gold/10">
                  <button 
                    ...
                    title={isMobile ? undefined : (isPlaying ? "إيقاف مؤقت" : "تشغيل")}
                  >
                  ...
                  <button 
                    ...
                    title={isMobile ? undefined : "سرعة التلاوة"}
                  >
                    {playbackRate}x
                  </button>
                  <button
                    ...
                    title={isMobile ? undefined : "تلاوة متواصلة"}
                  >
                    متواصل
                  </button>
                </div>
      ...
      <AnimatePresence>
        {!isFocusMode && (
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="glass-card p-3 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-3 shadow-lg"
          >
            {/* Left side: Theme & Surah Selection */}
            <div className="flex items-center gap-2 w-full md:w-auto">
              <button
                onClick={() => {
                  const isDark = document.documentElement.classList.toggle('dark');
                  localStorage.setItem('theme', isDark ? 'dark' : 'light');
                  // Force a re-render if needed, but classList toggle is usually enough for Tailwind
                }}
                className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors"
                title="تبديل الوضع الليلي"
              >
                <Moon className="w-5 h-5 text-primary dark:text-gold" />
              </button>
              ...
              <select 
                ...
                className="bg-white/50 dark:bg-black/40 border border-gray-200 dark:border-white/10 rounded-xl p-2 text-sm font-tajawal focus:ring-2 focus:ring-primary outline-none flex-1 md:flex-none"
              >
            ...
            {/* Right side: Ayah Jump & Controls */}
            <div className="flex flex-wrap items-center justify-center gap-2 w-full md:w-auto">
              <div className="flex items-center gap-2 bg-white/50 dark:bg-black/40 rounded-xl p-1 border border-gray-200 dark:border-white/10">
                <input 
                  type="number" 
                  placeholder="رقم الآية" 
                  value={targetAyah}
                  onChange={(e) => setTargetAyah(e.target.value)}
                  className="w-16 bg-transparent text-center text-sm outline-none font-tajawal"
                />
                <button 
                  onClick={handleAyahJump}
                  className="bg-primary dark:bg-primary-dark text-white px-4 py-1.5 rounded-lg text-sm font-bold hover:bg-primary-light transition-colors"
                >
                  انتقال
                </button>
              </div>
              ...
              <div className="flex items-center gap-1">
                <div className="flex items-center gap-1 bg-gray-100 dark:bg-white/10 rounded-full p-1 border border-gray-200 dark:border-white/5">
                  <button onClick={() => setFontSize(Math.max(18, fontSize - 2))} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-white dark:hover:bg-white/10 text-gray-500">-</button>
                  <span className="w-6 text-center text-xs font-bold">{fontSize}</span>
                  <button onClick={() => setFontSize(Math.min(48, fontSize + 2))} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-white dark:hover:bg-white/10 text-gray-500">+</button>
                </div>

                <button onClick={() => setIsFocusMode(true)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors text-gray-500" title="وضع التركيز">
                  <Maximize className="w-5 h-5" />
                </button>
                
                <button onClick={() => setIsSearchOpen(true)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/10 transition-colors text-gray-500" title="بحث">
          ...
          >
            ...
            >
              <div className="p-4 border-b border-gray-100 dark:border-white/10 flex items-center gap-3">
                ...
                />
                {searchQuery && (
                  <button onClick={() => handleSearch('')} className="p-1 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full text-gray-400">
                    <X className="w-4 h-4" />
                  </button>
                )}
                  ...
                  <div className="space-y-1">
                    {searchResults.map((res, idx) => (
                      <button
                        ...
                        className="w-full text-right p-4 hover:bg-primary/5 dark:hover:bg-gold/5 rounded-xl transition-all border border-transparent hover:border-primary/10 dark:hover:border-gold/20 group"
                      >
                        <div className="text-lg font-amiri text-primary dark:text-gold mb-2 group-hover:scale-[1.01] transition-transform">{res.text}</div>
                        <div className="flex items-center justify-end gap-3 text-xs text-gray-500 font-tajawal">
                          <span className="bg-gray-100 dark:bg-white/5 px-2 py-1 rounded-md">{res.surah.name}</span>
                          <span className="bg-gray-100 dark:bg-white/5 px-2 py-1 rounded-md">آية {res.numberInSurah}</span>
                          <span className="bg-gray-100 dark:bg-white/5 px-2 py-1 rounded-md">صفحة {res.page}</span>
                        </div>
                      </button>
                    ))}
                  </div>
              ...
              return (
                <React.Fragment key={ayah.number}>
                  ...
                  <span 
                    ...
                    title={isMobile ? undefined : "اضغط لإظهار خيارات الآية"}
                  >
                    {textWithoutLastWord}{' '}
